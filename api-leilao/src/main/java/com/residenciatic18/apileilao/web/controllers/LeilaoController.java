...
public class LeilaoController {
  ...
  public ResponseEntity<List<LeilaoResponseDto>> getById(@PathVariable Long id) {
    try {
      List<Leilao> leiloes = leilaoService.buscarTodos(id);

      if (leiloes.isEmpty()) {
        return ResponseEntity.notFound().build();
      }

      return ResponseEntity.ok(LeilaoMapper.toListDto(leiloes));
  ...
  public ResponseEntity<List<LeilaoResponseDto>> buscarTodos() {
    return ResponseEntity.ok(LeilaoMapper.toListDto(leilaoService.buscarTodos(null)));
  }
